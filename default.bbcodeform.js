				...
				_dropDown: function (editor, caller, action) {
					var $content = $('<div>')
						.append('<label for="link">URL:</label>',
							...
							'<label for="text_only">Только текст (без виджета)</label>', 
...
//                					var data = {'omit_script': "true", 'lang': "ru", 'url': encodeURIComponent(link)};
									...
									$.sceditor.command.get('twitter')._checkURL( $link.val(), data, function(result){
										...
										action(result.link, widget_type, hide_media, width, text_only, result.tweet);
										editor.closeDropDown(true);
										e.preventDefault();
										return;
									});
				...
				exec: function (caller) {
					...
					$.sceditor.command.get('twitter')._dropDown(editor, caller, function(link, widget_type, hide_media, width, text_only, tweet) {
						if( text_only ){
							...
							thtml = '<div contentEditable="false" class="twitter-div" style="width: ' + width + 'px;" widget_type="' + widget_type + '"' + hide_media + ' link="' + link + '" data-width="' + width + '"><blockquote class="twitter-' + widget_type + '" data-lang="ru" data-width="' + width + '"' + hide_media + ' data-conversation="none"><a href="' + link + '"><span class="twitter-logo"></span></a></blockquote></div><br />';
						}
			...
			$.sceditor.command.set("video", {
				_checkURL: function(url, callback){
					...
					// html5
					$.each([/^(https?:\/\/video\.twimg\.com\/tweet_video\/[a-z0-9\-\=\_]+\.mp4)/i],
						function(idx, reg){
							if ( (matches = url.match(reg)) ){
								videoURL = matches[1];
								return false;
							...
							.append('<div class="video_data_block" id="p2_title" style="display: block;">',
								...
								$('<input type="button" class="button" value="Вставить" />')
									.click(function(e){
										...
										action( vdata.type, vdata.id, frameSrc, vdata.link, width, height, $("#video_start").change().attr('intval') );
										editor.closeDropDown(true);
										e.preventDefault();
									})
							)
					...
					$.sceditor.command.get('video')._dropDown(editor, caller, function(vtype, videoId, frameSrc, videoLink, width, height, start) {
						...
						switch(vtype){
							...
							case 'facebook' :
								...
								break;
							case 'twitter' :
								vhtml = '<div contentEditable="false" class="twitter-div" style="width: ' + width + 'px;" widget_type="video" link="' + videoLink + '" data-width="' + width + '"><blockquote class="twitter-video" data-lang="ru" data-width="' + width + '" data-conversation="none"><a href="' + videoLink + '"><span class="twitter-logo"></span></a></blockquote></div><br />';
								break;
							case 'html5' :
